...
--2)
SELECT C.categoria, PR.produto, AVG(pd.preco_unit * pd.qtde ) AS Media
...
HAVING AVG(pd.preco_unit * pd.qtde  )> 1000
ORDER BY Media;
...
FROM cliente Cl INNER JOIN pedido P ON Cl.codcliente = P.codcliente
	...
	INNER JOIN produto ON produto.codproduto = pd.codproduto
	INNER JOIN categoria c ON c.codcategoria = produto.codcategoria
